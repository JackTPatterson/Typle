                ...
                <div>
                    {
                        Array.from(words).map((chars, i)=>{
                            return <span className={charStatus?.[i] === -1 ? `text-foreground opacity-50 text-2xl` : charStatus?.[i] === 0 ? 'opacity-100 text-2xl text-foreground' : 'text-red-500 line-through text-2xl'} key={`char_index_${i}`}>{chars}</span>
                        })
                    }
                </div>
